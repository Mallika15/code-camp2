...
# Edit restaurant details, ex.: /myrestaurants/restaurants/1/edit/
    path(r'\^restaurants/(?P<pk>\d+)/edit/\$',
        UpdateView.as_view(
        	model = Restaurant,
...
# Create a restaurant dish, ex.: /myrestaurants/restaurants/1/dishes/create/
    path(r'\^restaurants/(?P<pk>\\d+)/dishes/create/\$',
    	DishCreate.as_view(),
        name='dish_create'),

# Edit restaurant dish details, ex.: /myrestaurants/restaurants/1/dishes/1/edit/
    path(r'\^restaurants/(?P<pkr>\\d+)/dishes/(?P<pk>\\d+)/edit/\$',
    	UpdateView.as_view(
    		model = Dish,
    		template_name = 'myrestaurants/form.html',
    		form_class = DishForm),
...
# Unlike the previous patterns, this one is implemented using a method view instead of a class view
    path(r'\^restaurants/(?P<pk>\\d+)/reviews/create/\$',
    	review,
    	name='review_create'),
]